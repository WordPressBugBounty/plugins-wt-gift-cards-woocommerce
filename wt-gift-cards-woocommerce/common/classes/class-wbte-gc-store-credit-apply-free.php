...
class Wbte_Gc_Store_Credit_Apply_Free extends Wbte_Woocommerce_Gift_Cards_Free_Common {
	...
	public function update_credit_amount( $order ) {
		...
		$display_prices_including_tax = get_option('woocommerce_prices_include_tax') === 'yes';
		...
		/**
         *  Woocommerce 8.9.1 Compatibility
         *  'woocommerce_new_order' is triggered and thus deducting the coupon amount on Block checkout.
         *   And duplicate deduction of wallet amount on Block checkout
         */
        if(  !is_admin() && 'woocommerce_new_order' === current_filter() 
		...
		$store_credit_used      = array();
		$coupon_discount_totals = $this->get_coupon_discount_totals($order);
		$coupon_discount_tax_totals = $this->get_coupon_discount_tax_totals($order);
	...
	*/
     ...
     */
    public function get_coupon_discount_totals($order = null)
    {
        if( is_admin() && $order )
        {
            foreach ( $order->get_items( 'coupon' ) as $item_id => $item ) {
                $coupon_code = $item->get_code();
                $coupon_discount_totals[$coupon_code] = $item->get_discount();
            }
     ...
     *  @since 1.2.2
     */
    public function get_coupon_discount_tax_totals( $order = null )
    {
        if( is_admin() && $order )
        {
            ...
            foreach ( $order->get_items( 'coupon' ) as $item_id => $item) {
                $coupon_code = $item->get_code();
                $coupon_discount_tax_totals[$coupon_code] = $item->get_discount_tax();
            }
        }elseif( ! empty( WC()->cart ) )
        {
