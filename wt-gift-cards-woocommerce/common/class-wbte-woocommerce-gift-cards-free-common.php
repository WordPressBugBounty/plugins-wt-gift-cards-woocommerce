...
class Wbte_Woocommerce_Gift_Cards_Free_Common {
	...
	public function __construct( $plugin_name, $version ) {
		...
		/**
		 *  @since 1.2.3
		 *  Shortcode for checking gift card balance
		 */
		add_shortcode('wt_gc_check_gift_card_balance',array($this, 'check_gift_card_balance_shortcode'));
		
		/**
		 *  @since 1.2.3
		 *  Ajax hook for displaying Gift card balance 
		 */
		add_action('wp_ajax_wbte_gc_check_balance', array($this, 'check_gift_card_balance_ajax'));
        add_action('wp_ajax_nopriv_wbte_gc_check_balance', array($this, 'check_gift_card_balance_ajax'));
	}
	...
	/**
	 *  Print gift card balance shortcode content
	 * 
	 *  @since  1.2.3
	 *  @return bool|string  Balance check form content
	 */
	public function check_gift_card_balance_shortcode() {
        ...
        ?>
			...
			<div class="wbte_gc_check_balance_form_wrapper">
				<div class="wbte_gc_check_balance_form_inner">
					...
					<form class="wbte_gc_check_balance_form">
						...
						<?php if($restricted_email_label['enable_email_field']): ?>
						
							<label for="wbte_gc_restricted_email"><?php echo esc_html($restricted_email_label['email_field_subtitle']); ?></label>
							<input type="email" name="wbte_gc_restricted_email" class="wbte_gc_restricted_email">
	 ...
	 /**
     *  Checking and displaying gift card balance Ajax callback.
     *  
     *  @since 1.2.3
     */
	public function check_gift_card_balance_ajax() {
        
		$out = array(
			'balance' => '',
			'message'    => esc_html__( 'Error', 'wt-gift-cards-woocommerce' ),
		);
        ...
        $restricted_email = isset($_POST['restricted_email']) ? sanitize_text_field(wp_unslash($_POST['restricted_email'])) : '';
		
		$coupon = new WC_Coupon($coupon_code);
        if (!$coupon->get_id() || $coupon->get_discount_type() !== 'store_credit' ) {
			$out = array(
				'balance' => '',
				'message'    => esc_html__("We couldn't find a gift card associated with that code. Please verify the code and try again.", 'wt-gift-cards-woocommerce'),
			);
			wp_send_json_error($out);
			exit();
        }
        ...
        if(!empty($coupon_email_restrictions) && is_array($coupon_email_restrictions)) {
			if(empty($restricted_email) ) {
				$out = array(
					'balance' => '',
					'message'    => esc_html__( 'This gift card is linked to an email address. Please enter the email address associated with it!', 'wt-gift-cards-woocommerce' ),
				);
				wp_send_json_error($out);
				exit();
            }
            elseif(!Wbte_Woocommerce_Gift_Cards_Free_Common::is_coupon_emails_allowed(array($restricted_email),$coupon)) {
				
				$out = array(
					'balance' => '',
					'message'    => esc_html__( 'The email address you entered is not associated with the gift card. Please check the email address and try again.', 'wt-gift-cards-woocommerce' ),
				);
				wp_send_json_error($out);
				exit();
            }
