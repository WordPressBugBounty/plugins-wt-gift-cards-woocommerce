 ...
 */
define( 'WBTE_GC_FREE_VERSION', '1.2.5' );
...
if ( ! class_exists( 'WooCommerce' ) ) {
	...
	wp_die( esc_html__( 'Oops! Woocommerce not activated, It should required for `Gift cards for woocommerce`.', 'wt-gift-cards-woocommerce' ), '', array( 'back_link' => 1 ) );
}

// Display update information for the plugin.
add_action('in_plugin_update_message-wt-gift-cards-woocommerce/wt-gift-cards-woocommerce.php', 'wbte_gc_update_message', 10, 2 );

if ( ! function_exists( 'wbte_gc_update_message' ) ) {
    ...
    function wbte_gc_update_message( $data, $response )
    {
        if ( isset( $data['upgrade_notice'] ) )
        {
            add_action( 'admin_print_footer_scripts', 'wbtegc_plugin_screen_update_notice_js' );
            $msg = str_replace( array( '<p>', '</p>' ), array( '<div>', '</div>' ), $data['upgrade_notice'] );
            echo '<style type="text/css">
            #wt-gift-cards-woocommerce-update .update-message p:last-child{ display:none;}     
            #wt-gift-cards-woocommerce-update ul{ list-style:disc; margin-left:30px;}
            .wbtegc_update_message{ padding-left:20px;}
            </style>
            <div class="update-message wbtegc_update_message">' . wp_kses_post( wpautop( $msg ) ) . '</div>';
        }
    }
}

if ( ! function_exists( 'wbtegc_plugin_screen_update_notice_js' ) ) {
    /**
     *  @since 1.1.4
     *  Javascript code for upgrade notice in plugins page.
     */
    function wbtegc_plugin_screen_update_notice_js() 
    {   
        global $pagenow;
        ...
        <script>
            ( function( $ ){
                var update_dv=$('#wt-gift-cards-woocommerce-update');
                update_dv.find('.wbtegc_update_message').next('p').remove();
                update_dv.find('a.update-link:eq(0)').on('click', function(){
                    $('.wbtegc_update_message').remove();
                });
            })( jQuery );
        </script>
        <?php
